...
namespace Colsp.Api.Controllers
{
	public class ProductStagesController : ApiController
	{
		...
		private CmosService cmosService = new CmosService();
		private ProductStageService productStageService = new ProductStageService();
...

		{
			try
			{
				...
				Util.DeadlockRetry(db.SaveChanges, "ProductStage");
				elasticService.SendToElastic(stageGroup, Apis.ElasticCreateProduct, "POST", email, currentDt, db);
		...
		public HttpResponseMessage SaveChangeAllImage(List<VariantRequest> request)
		{
			try
			{
				...
				{
					productStageService.SetupStageAfterSave(stage,email,currentDt,db, false);
				}
				db.Configuration.ValidateOnSaveEnabled = false;
				Util.DeadlockRetry(db.SaveChanges, "ProductStage");
		...
		public HttpResponseMessage AddProduct(ProductStageRequest request)
		{
			try
			{
				...
				var email = User.UserRequest().Email;
				var currentDt = SystemHelper.GetCurrentDateTime();
				productStageService.SetupProductStageGroup(group, request, attributeList, inventoryList, shop.ShopId, false, true, email, currentDt, db);
				...
				productStageService.SetupGroupAfterSave(group,email,currentDt, db, true);
				Util.DeadlockRetry(db.SaveChanges, "ProductStage");
				//send to cmos
				cmosService.SendToCmos(group, Apis.CmosCreateProduct, "POST", email, currentDt, db);
		...
		public HttpResponseMessage SaveChangeProduct([FromUri]long productId, ProductStageRequest request)
		{
			try
			{
				...
				var email = User.UserRequest().Email;
				var currentDt = SystemHelper.GetCurrentDateTime();
				productStageService.SetupProductStageGroup(group, request, attributeList, inventoryList, group.ShopId, isAdmin, false, email, currentDt, db);
				#region validate url key
				...
				#endregion
				AutoGenerate.GeneratePid(db, group.ProductStages);
				productStageService.SetupGroupAfterSave(group,email,currentDt, db, false);
				//save change database
				Util.DeadlockRetry(db.SaveChanges, "ProductStage");
				//send to cmos
				cmosService.SendToCmos(group, Apis.CmosUpdateProduct, "PUT", email, currentDt, db);
				if (Constant.PRODUCT_STATUS_APPROVE.Equals(group.Status))
				{
					//send to elastic search
					elasticService.SendToElastic(group, Apis.ElasticCreateProduct, "POST", group.UpdateBy, group.UpdateOn, db);
				}
				else if (!string.IsNullOrEmpty(group.ApproveBy))
				{
					elasticService.UpdateByPassFieldElastic(group,Apis.ElasticUpdateProduct,"PUT",email,currentDt, db);
				}
				#endregion
				return GetProductStage(group.ProductId);
			}
		...
		public HttpResponseMessage GetProductStage(long productId)
		{
			try
			{
				ProductStageRequest response = productStageService.GetProductStageRequestFromId(db, productId);
				return Request.CreateResponse(HttpStatusCode.OK, response);
			}
			catch (Exception e)
			{
		...
		public HttpResponseMessage ApproveProduct(List<ProductStageRequest> request)
		{
			try
			{
				...
				foreach (ProductStageRequest proRq in request)
				{
					...
					productStageService.SetupApprovedProduct(pro, db);
				}
				Util.DeadlockRetry(db.SaveChanges, "ProductStage");
				return Request.CreateResponse(HttpStatusCode.OK);
			}
		...
		public HttpResponseMessage DuplicateProductStage(long productId)
		{
			try
			{
				...
				var response = productStageService.GetProductStageRequestFromId(db, productId);
				if (response == null)
				{
