...
namespace Colsp.Api.Controllers
{
    public class ProductStagesController : ApiController
    {
            ...
            try
            {
                ...
                var products = (from p in db.ProductStageGroups
                                ...
                                select new
                                {
                                    ...
                                    ImageUrl = string.Empty.Equals(p.ProductStages.FirstOrDefault().FeatureImgUrl) ? string.Empty : string.Concat(Constant.IMAGE_STATIC_URL, p.ProductStages.FirstOrDefault().FeatureImgUrl),
        ...
        public HttpResponseMessage GetProductStages([FromUri] ProductRequest request)
        {
            try
            {
                ...
                var products =  db.ProductStages.Where(w => w.IsVariant == false && !Constant.STATUS_REMOVE.Equals(w.Status)).Select(s => new
                {
                    ...
                    s.VariantCount,
                    s.IsMaster,
                    ImageUrl = string.Empty.Equals(s.FeatureImgUrl) ? string.Empty : string.Concat(Constant.IMAGE_STATIC_URL,s.FeatureImgUrl),
        ...
        private void SendToElastic(ProductStageGroup group, string url, string method
            , string email, DateTime currentDt, ColspEntities db)
        {
            ...
            foreach (var stage in group.ProductStages)
            {
                ElasticRequest cms = new ElasticRequest()
                {
                    ...
                    Bu = stage.Bu,
                    GlobalCatId = group.GlobalCatId,
                    //GlobalCatNameEn = stage,
                    //GlobalCatNameTh = stage,
                    //GlobalRootCatId = stage,
                    //GlobalRootCatNameEn = stage,
                    //GlobalRootCatNameTh = stage,
                    LocalCatId = group.LocalCatId,
                    //LocalCatNameEn = stage,
                    //LocalCatNameTh = stage,
                    //LocalRootCatId = stage,
                    //LocalRootCatNameEn = stage,
                    //LocalRootCatNameTh = stage,
                    BrandId = group.BrandId,
                    //BrandNameEn = stage,
                    //BrandNameTh = stage,
                    ProductNameEn = stage.ProductNameEn,
                    ...
                    ImageCount = stage.ImageCount,
                    Installment = stage.Installment,
                    EffectiveDate = group.EffectiveDate.ToString("yyyy-MM-ddTHH:mm:ss.fffZ"),
                    ExpireDate = group.ExpireDate.ToString("yyyy-MM-ddTHH:mm:ss.fffZ"),
                    ...
                    //IsLocalSearch = stage,
                    //IsGlobalSearch = stage,
                    //IsSearch = stage,
                    //StockAvailable = stage,
                    tag = group.ProductStageTags.Select(s => s.Tag).ToList(),
                };
                requests.Add(cms);
            }
            foreach (var req in requests)
            {
                ...
                var json = new JavaScriptSerializer().Serialize(tmp);
